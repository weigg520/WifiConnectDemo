...
public class WifiAdmin {
    ...
    /**
     * @descriptoin	判断wifi是否打开
     * @author	    dc
     * @param
     * @date 2016/8/12 10:57
     * @return false :没打开   true:打开
     */
    ...
    /** * 添加指定WIFI的配置信息,原列表不存在此SSID * * @param wifiList * @param ssid * @param pwd * @return */
    public int AddWifiConfig1(String ssid, String pwd) {
        int wifiId = -1;
        Log.i("AddWifiConfig", "equals");
        WifiConfiguration wifiCong = new WifiConfiguration();
    ...
    /**
     * 获取已root设备的当前链接的wifi密码
     * @param wifiName
     * @return
     */
    public String getWifiPsd(String wifiName) {
        ...
        try {
            ...
            dos.writeBytes("cat /data/misc/wifi/*.conf\n");
            dos.writeBytes("exit\n");
            dos.flush();
        ...
        Pattern p = Pattern.compile("network=\\{([^\\}]+)\\}", Pattern.DOTALL);
        Matcher m = p.matcher(wifiConf.toString());
        while (m.find()) {
            String networkBlock = m.group();
            Pattern p2 = Pattern.compile("ssid=\"([^\"]+)\"");
            Matcher m2 = p2.matcher(networkBlock);

            if (m2.find()) {
                if (wifiName.equals(m2.group(1))) {
